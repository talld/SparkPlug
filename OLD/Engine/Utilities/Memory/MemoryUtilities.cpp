...
{
	return getUBOAllignment(structSize, 256);
}
...
unsigned int getUBOBlockSize(size_t structSize, VkDeviceSize minUBOAllocation)
{
	...
	//TODO: smart bitwise maths; bit shifting maybe?

	blockCount = std::ceil((double)structSize / (double)minUBOAllocation);

	return blockCount;
}

void* allocateDynamicBufferTransferSpace(size_t UBOAllignment) {

	// maybe circ buffer this?
	// or double size every alloc to a max
	// really anything but this
	void* modelTransferSpace = (void*)_aligned_malloc(UBOAllignment * memUtilitesSettings.MAX_OBJECTS, UBOAllignment); // this will have to be changed into its own object once i actualy know what im doing 
																													   // most likely a generic UBO class with takes its place				
