	...
	for (size_t i = 0; i < commandBuffers.size(); i++) {
		...
		vkCmdBeginRenderPass(commandBuffers[i], &renderPassBeginInfo, VK_SUBPASS_CONTENTS_INLINE);
			...
			for (size_t j = 0; j < meshes->size(); j++) {
				...
				uint32_t dynamicOffset = static_cast<uint32_t>(getUBOAllignment(sizeof(MeshUBO))) * j;

				vkCmdBindDescriptorSets(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipeline.getPipeline().layout, 0, 1, camera->getDescriptorSet(i), 1, &dynamicOffset);
